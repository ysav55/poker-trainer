...
// ─── BlindPresetRepository ────────────────────────────────────────────────────

const BlindPresetRepository = {
  /**
  ...
  async list(schoolId) {
    const systemQuery = supabase
      .from('blind_structure_presets')
      .select('id, school_id, name, description, levels, is_system, created_by, created_at')
      .eq('is_system', true)
   ...
   * Returns the new row.
   */
  async create({ name, description = null, levels, schoolId, createdBy }) {
    const data = await q(
      supabase
        .from('blind_structure_presets')
        .insert({
...
module.exports = { BlindPresetRepository };
