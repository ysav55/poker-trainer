/* LiveConfigureHand.jsx
   Single card combining:
     • Deal mode (RNG / Manual / Hybrid)
     • Per-player hole-card picker — Cards (52-grid) OR Range (13×13 matrix)
     • Board picker — specific cards + texture tags (composable)

   Server contract:
     emit('set_mode', { mode })
     emit('manual_deal_card', { targetType:'player', targetId, position, card })
     emit('manual_deal_card', { targetType:'board',  position, card })
     emit('set_player_range',  { stableId, range })
     emit('set_board_texture', { tags })
*/

const { useState: useCH, useMemo: useCHMemo } = React;

const CH_RANKS = ['A','K','Q','J','T','9','8','7','6','5','4','3','2'];
const CH_SUITS = ['s','h','d','c'];
const SUIT_GLYPH = { s:'♠', h:'♥', d:'♦', c:'♣' };

// Texture groups (lifted from server's HandConfigPanel)
...
/* ─────────────────────────────────────────────────────────── */
function ConfigureHand({ data }) {
  const { gameState } = data;

  const [mode, setMode] = useCH('hybrid');
  const [target, setTarget] = useCH(null);   // 'board' | stableId
  const [slot, setSlot] = useCH(0);          // active slot index
  const [inputMode, setInputMode] = useCH({}); // per stableId: 'cards' | 'range'
  const [granted, setGranted] = useCH({});   // { stableId: [c1,c2] }
  const [ranges,  setRanges]  = useCH({});   // { stableId: rangeMatrixMap }
  const [board,   setBoard]   = useCH([null,null,null,null,null]);
  const [textures, setTextures] = useCH([]); // active texture tags

  // What's already used (board + granted)
  const usedCards = useCHMemo(() => {
    const used = new Set(board.filter(Boolean));
    Object.values(granted).forEach(pair => pair.forEach(c => c && used.add(c)));
    // include any real dealt hole cards (non-HIDDEN)
  ...
  function toggleTexture(tag) {
    setTextures(prev => {
      ...
      // Radio within group: replacing same group, toggling off if already on
  ...
  return (
    <div className="card">
      <div className="card-head">
        ...
        <div className="card-kicker">{playerKicker}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 9 }}>
            ...
            >
              ...
              {set > 0 && (
                <span style={{
                  ...
                  color: isOn ? '#1a1208' : 'var(--ok)',
                }}>{set}/2</span>
              )}
              {hasRange && !set && (
                <span style={{
                  fontFamily: 'var(--mono)', fontSize: 9,
                  color: isOn ? '#1a1208' : 'var(--info)',
                }}>{rangeSize}c</span>
              )}
            </button>
          );
        ...
        >
          ...
          {board.filter(Boolean).length > 0 && (
            <span style={{
              fontFamily: 'var(--mono)', fontSize: 9,
              color: target === 'board' ? '#1a1208' : 'var(--ok)',
            }}>{board.filter(Boolean).length}/5</span>
          )}
        </button>
      </div>

      {/* Detail pane for active target */}
      {target && (
        <div style={{
          background: 'var(--bg-3)',
          ...
          {/* Slot indicator + cards/range toggle */}
          ...
          {(targetIsPlayer ? targetMode === 'cards' : true) && (
            <div className="row" style={{ gap: 4, marginBottom: 9 }}>
              {(targetIsPlayer ? [0,1] : [0,1,2,3,4]).map(i => {
                ...
                return (
                  ...
                  >
                    ...
                    {card ? <window.MiniCard code={card}/> : (
              ...
              <button
                ...
                onClick={clearTarget}
                title="Clear this target"
              >×</button>
            </div>
          )}

          {/* Card grid OR range matrix */}
          ...
          {/* Texture tags — only shown for board target */}
          {target === 'board' && (
      ...
      <div className="row" style={{ gap: 6, marginTop: 9 }}>
        ...
        <button className="btn sm primary" style={{ flex: 1.4 }}>Apply Next Hand</button>
      </div>
    </div>
  );
}
...
function RangeMatrix({ selected, onToggle }) {
  // selected: { 'AKs': true, ... }
  const totalCombos = useCHMemo(() => {
    let n = 0;
    for (const k in selected) {
      if (k.length === 2) n += 6;          // pair
      else if (k.endsWith('s')) n += 4;    // suited
      else n += 12;                        // offsuit
    }
    return n;
  }, [selected]);
    ...
    if (q.combos === 'all_suited') {
      for (let i = 0; i < CH_RANKS.length; i++) {
        for (let j = 0; j < CH_RANKS.length; j++) {
          ...
          else if (i === j) {} // skip pairs for suited preset
        }
      }
    } else {
      q.combos.forEach(c => { next[c] = true; });
    }
    // overwrite via toggling
  ...
  return (
    <>
      <div style={{
        ...
        {CH_RANKS.map((r1, i) =>
          CH_RANKS.map((r2, j) => {
            // i<j → suited (above diagonal), i>j → offsuit, i==j → pair
            let combo;
            if (i === j) combo = r1 + r2;
      ...
      <div className="row between" style={{ marginTop: 7 }}>
        ...
        <span style={{
          ...
          color: 'var(--accent)',
        }}>{totalCombos} combos · {pct}%</span>
      </div>
    </>
  );
}

window.ConfigureHand = ConfigureHand;
