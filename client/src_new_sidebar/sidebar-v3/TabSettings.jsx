/* Tab 5 · Settings
   Per spec:
   • Blinds Level — SB/BB editor + preset levels + timer (tournament blinds-up)
   • Seat Config — interactive seat map (click seat → edit/kick/sit-out)
   • Players — list of seated players with Stack / Kick / Force Sit-Out / Add Bot */

const { useState: useStateSet } = React;

function TabSettings({ data }) {
  const [section, setSection] = useStateSet('blinds');
...
function BlindsSection({ data }) {
  const [sb, setSb] = useStateSet(data.blindLevels.current.sb);
  const [bb, setBb] = useStateSet(data.blindLevels.current.bb);
  const [ante, setAnte] = useStateSet(data.blindLevels.current.ante);
  const [timerOn, setTimerOn] = useStateSet(data.blindLevels.timer.enabled);
    ...
    <>
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
          <div>
            <span className="lbl">SB</span>
            <input className="field" type="number" value={sb} onChange={e => setSb(+e.target.value)} />
          </div>
          <div>
            <span className="lbl">BB</span>
            <input className="field" type="number" value={bb} onChange={e => setBb(+e.target.value)} />
          </div>
          <div>
            <span className="lbl">Ante</span>
            <input className="field" type="number" value={ante} onChange={e => setAnte(+e.target.value)} />
          </div>
        </div>
        ...
        {data.blindLevels.presets.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              {active
                ? <span className="chip active">current</span>
                : <span className="chip ghost">use</span>}
            </div>
          );
        })}
      </div>
      ...
      <div className="card">
        <div className="card-head">
          ...
          <div className="card-kicker" style={{ color: timerOn ? 'var(--accent)' : 'var(--ink-faint)' }}>
            {timerOn ? 'ON' : 'OFF'}
          </div>
        </div>
        <div className="row between" style={{ marginBottom: 10 }}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--accent)' }}>
            {data.blindLevels.timer.levelMinutes} min
          </div>
        </div>
        <div className="row between" style={{ marginBottom: 10 }}>
        ...
        <div className="row" style={{ gap: 6 }}>
          <button
            className={'btn full' + (timerOn ? ' primary' : '')}
            onClick={() => setTimerOn(t => !t)}
          >{timerOn ? 'Pause Timer' : 'Start Timer'}</button>
...
function SeatsSection({ data }) {
  const [selected, setSelected] = useStateSet(0);
  const seat = data.seatConfig.seats[selected];

  // Hex grid — 9 seats arranged in a 3x3 with a header
  return (
    <>
      <div className="card">
        <div className="card-head">
            ...
            return (
              ...
              >
                <span style={{
                  ...
                  color: occupied ? 'var(--accent)' : 'var(--ink-faint)',
                }}>S{i+1}</span>
                <span style={{
                  fontSize: 10.5, fontWeight: 600,
                  color: occupied ? 'var(--ink)' : 'var(--ink-faint)',
      ...
      <div className="card">
        ...
        {seat.player ? (
          <>
            ...
            }}>Stack {seat.stack} · {seat.status}</div>
