/* Shared primitives used across tabs.
   Exposed on window so other Babel script files can pick them up. */

const { useState, useEffect, useRef } = React;

/* ─── Tiny card glyph (matches server "As","Td","HIDDEN") ──── */
function MiniCard({ code, ghost }) {
  if (ghost) return <span className="mini-card ghost">·</span>;
  if (!code || code === 'HIDDEN') {
    return <span className="mini-card" style={{
      background: 'repeating-linear-gradient(45deg, rgba(201,163,93,0.15) 0 2px, transparent 2px 5px)',
      borderStyle: 'dashed',
    }}>?</span>;
  }
  const rank = code.slice(0, -1);
  const suit = code.slice(-1);
  const suitGlyph = { s: '♠', h: '♥', d: '♦', c: '♣' }[suit.toLowerCase()] || suit;
  const red = suit.toLowerCase() === 'h' || suit.toLowerCase() === 'd';
  return (
    <span className={'mini-card' + (red ? ' red' : '')}>
      {rank}{suitGlyph}
    </span>
  );
}

/* ─── Status dot + label for sb-head ──────────────────────── */
function StatusPill({ state }) {
  // state: 'live' | 'paused' | 'scenario' | 'review'
  const map = {
    live:     { color: '#4ad991', label: 'LIVE' },
    paused:   { color: '#f5b25b', label: 'PAUSED' },
    scenario: { color: '#9b7cff', label: 'SCENARIO' },
    review:   { color: '#6aa8ff', label: 'REVIEW' },
  };
  const { color, label } = map[state] || map.live;
  return (
    <div className="sb-status" style={{ color }}>
      <span className="dot" style={{ background: color, boxShadow: `0 0 8px ${color}` }}/>
      {label}
    </div>
  );
}

/* ─── Segmented control ──────────────────────────────────── */
function Segmented({ options, value, onChange, cols }) {
  return (
    <div className="segmented" style={{ gridTemplateColumns: `repeat(${cols || options.length}, 1fr)` }}>
      {options.map(o => (
        <button
          key={o.value ?? o}
          className={(value === (o.value ?? o)) ? 'active' : ''}
          onClick={() => onChange(o.value ?? o)}
        >{o.label ?? o}</button>
      ))}
    </div>
  );
}

/* ─── Header (logo + state) ──────────────────────────────── */
function Head({ status, subtitle }) {
  return (
    <div className="sb-head">
      <div className="sb-logo">
        FeltSide
        <small>{subtitle || 'Coach · Ariela'}</small>
      </div>
      <StatusPill state={status} />
    </div>
  );
}

/* ─── Tab bar ─────────────────────────────────────────────── */
function TabBar({ tab, onTabChange }) {
  const TABS = [
    { id: 'live',     label: 'Live' },
    { id: 'drills',   label: 'Drills' },
    { id: 'history',  label: 'History' },
    { id: 'review',   label: 'Review' },
    { id: 'settings', label: 'Setup' },
  ];
  return (
    <div className="sb-tabs">
      {TABS.map(t => (
        <div
          key={t.id}
          className={'sb-tab' + (tab === t.id ? ' active' : '')}
          onClick={() => onTabChange(t.id)}
        >{t.label}</div>
      ))}
    </div>
  );
}

Object.assign(window, { MiniCard, StatusPill, Segmented, Head, TabBar });
